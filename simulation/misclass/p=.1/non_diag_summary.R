...
for (i in c(1,2,3)){
  ...
  setwd("..")
  misclass.non_diag.list[[i]] = do.call(rbind, myresult.list)
}
result.matrix.misclass = round(do.call(rbind, misclass.non_diag.list),3)[,c(3,4,7,8)]
...
for (i in c(1,2,3)){
  ...
  setwd("..")
  trueclass.non_diag.list[[i]] = do.call(rbind, myresult.list)
}
result.matrix.trueclass = round(do.call(rbind, trueclass.non_diag.list),3)[,c(3,4,7,8)]
...
setwd("./non_diag_cov")
comp.non_diag.list =list()
for (i in c(1,2,3)){
  result.mtx = read.csv(paste0("simulation_results3", i, ".csv"), header = T)
  mymean = apply(result.mtx, 2, function(x) mean(x, na.rm = T))
  mysd = apply(result.mtx, 2, function(x) sd(x, na.rm = T))
  comp.non_diag.list[[i]] = cbind(t(rbind(mymean, mysd)[,1:4]), t(rbind(mymean, mysd)[,5:8]))
}
result.matrix.comp = round(do.call(rbind,comp.non_diag.list),3)
...
setwd("C:/Users/peiyao/Documents/GitHub/LWPR/simulation/misclass/p=.1")
write.table(do.call(rbind, result.list), file = "non_diag_p=.1.csv", sep = ',', col.names = T, row.names = T)
